...
#   WATCH_DIRS="src"      # 监听目录(空格分隔),默认 src;可加 assets 等
#
# 说明:
#   - 默认 debug 增量编译:只重编你改动的 crate,几秒~几十秒;release 每次都要重做全量 LTO,数分钟。
#   - 自动重启会先 kill 掉正在跑的实例再拉起新窗口(避免 ETXTBSY:运行中的二进制不能覆盖)。
#   - 产物拷贝到工作区 $ROOT/target/dev（与 /tmp 的 cargo 目标目录分开,避免在 FUSE 上反复触碰）。
set -euo pipefail

APP="apipost-rs"            # 二进制/包名
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "$ROOT"

TARGET_DIR="${CARGO_TARGET_DIR:-/tmp/apipost-rs-target}"
RUN_BIN="$ROOT/target/dev/$APP"
...
if [ -n "$PROFILE" ]; then
  BUILT="$TARGET_DIR/release/$APP"
else
  BUILT="$TARGET_DIR/debug/$APP"
fi
...
# 杀死正在运行的实例(PID 文件 + 按进程名 $APP,含 cargo run 启动的),等它退出
kill_running() {
  if [ -f "$PID_FILE" ]; then
